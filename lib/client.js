...
storage = module.exports.storage = require('node-persist');
...
init = module.exports.init = function (callback) {
    ...
    var persistedQueue = storage.getItem('queue');
    if (persistedQueue) {
        persistedQueue.forEach(function (element) {
            queue.enq(element);
        });
    }
...
function saveQueue() {
    storage.setItem('queue', queue._elements);
}
